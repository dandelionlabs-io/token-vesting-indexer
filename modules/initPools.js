...
const vestingPoolABI = JSON.parse(
  fs.readFileSync("abis/vesting.json", "utf-8")
);
...
const initFactory = async (factoryInfo) => {
  ...
  if (!web3list.get(factoryInfo.network)) {
    ...
    web3list.set(
      factoryInfo.network,
      blockchain.reInit(networkInformation.rpcUrl)
    );
  }
  ...
  if (!factory) {
    ...
    console.log(
      `Created new factory instance in database: ${factory.network} - ${factory.address}`
    );
  }
...
const initPools = async () => {
  ...
  for (let factoryIndex = 0; factoryIndex < factories.length; factoryIndex++) {
    ...
    for (let i = 0; i < addresses.length; i++) {
      ...
      const poolContract = blockchain.loadContract(
        web3list.get(factory.network),
        poolAddress,
        vestingPoolABI
      );
