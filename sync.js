...
let log = async function(conf, latestBlockNum, syncedBlockHeight) {
    ...
    for (let i = 0; i < iterationCount; i++) {
      ...
      await processEvents(privateSale, 'private-sale', conf, from, to);

      // To decrease pressure on Node, wait for 1 second.
      await timeOut(1);

      /// Fetch events for Chain Guardian
      await processEvents(chainGuardian, 'chain-guardian', conf, from, to);

      // To decrease pressure on Node, wait for 1 second.
      await timeOut(1);

      /// Fetch events for Trust Pad
      await processEvents(trustPad, 'trust-pad', conf, from, to);
